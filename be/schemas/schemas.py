...
class TripOut(BaseModel):
    ...
    places_route: Optional[str] = None
    created_at: datetime
    model_config = {"from_attributes": True}
...
class RoadmapPrecomputeOut(BaseModel):
    total: int
    queued: int
    skipped_fresh: int
    skipped_invalid: int
...
class RoadmapTripStatusOut(BaseModel):
    trip_id: int
    trip_name: str
    state: str
    ai_ready: bool
...
class RoadmapStatusOut(BaseModel):
    total: int
    fresh: int
    stale: int
    invalid: int
    items: list[RoadmapTripStatusOut]
    ai_available: bool
