    ...
    for _ in range(5):
        ...
        async with httpx.AsyncClient(timeout=AI_HTTP_TIMEOUT) as client:
            ...
            if res.status_code == 429:
                # Rate-limited — wait and retry (up to 3 times within the outer loop)
                _rate_retries = getattr(call_gemini_with_tools, "_rate_retries", 0)
                wait_s = min(8, 2 ** _rate_retries)
                logging.getLogger("travelmate.ai").warning(
                    "Gemini tools 429 rate-limit, retrying in %ds", wait_s
                )
                await asyncio.sleep(wait_s)
                continue  # retry the outer for-loop iteration
...
async def call_gemini(prompt: str) -> str:
    ...
    for attempt in range(3):
        ...
        if res.status_code == 429:
            ...
            _log.warning("Gemini 429 rate-limit (attempt %d/3), retrying in %ds", attempt + 1, wait)
            if attempt < 2:
    ...
    raise HTTPException(429, "Gemini rate limit persists. Please wait a minute and try again.")
...
async def recommend(body: AIRequest, u: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    if gemini_ok:
        try:
            ...
            # On rate-limit or server errors, gracefully fall back to cheaper providers
            if exc.status_code in (429, 500, 502, 503, 504):
                fallback_prompt = prompt[:4000]
                if groq_ok:
                    _log.warning("Gemini tools failed (%s) for /recommend, falling back to Groq", exc.status_code)
                    response = await call_groq(fallback_prompt)
                elif grok_ok:
                    _log.warning("Gemini tools failed (%s) for /recommend, falling back to Grok", exc.status_code)
                    response = await call_grok(fallback_prompt)
                else:
                    raise
            else:
                raise
