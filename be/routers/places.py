...
def _get_from_cache(trip_id: int) -> List[Place] | None:
    cache_entry = _places_cache.get(trip_id)
    if cache_entry and (time.time() - cache_entry['timestamp']) < CACHE_TTL:
        return cache_entry['data']
    return None

def _set_cache(trip_id: int, data: List[Place]):
    _places_cache[trip_id] = {
        'data': data,
        'timestamp': time.time()
    }

def _invalidate_cache(trip_id: int):
    _places_cache.pop(trip_id, None)
...
async def list_places(trip_id: int, u: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    # Check cache first
    cached = _get_from_cache(trip_id)
    if cached:
        return cached
...
async def create_place(trip_id: int, body: PlaceIn, u: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    _invalidate_cache(trip_id)  # Invalidate cache after create
    return place
...
async def update_place(trip_id: int, place_id: int, body: PlaceUpdate, u: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    await db.flush()
    await db.refresh(place)
    _invalidate_cache(trip_id)  # Invalidate cache after update
    return place
...
async def delete_place(trip_id: int, place_id: int, u: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    await db.flush()
    _invalidate_cache(trip_id)  # Invalidate cache after delete
...
async def optimize_route(trip_id: int, u: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    optimized_places = all_r.scalars().all()
    _set_cache(trip_id, optimized_places)  # Cache optimized results
